...
pthread_mutex_t lock;//pthread库的一个线程锁
...
void app_thread_exit(int ret, int fd)//关闭线程连接和socket
{
	close(fd);
...
int app_connect(int type, void* buf, unsigned short int portnum)//建立线程连接
{
	...
	if (type == IP) {
		...
		snprintf(address, ARRAY_SIZE(address), "%hhu.%hhu.%hhu.%hhu",
			ip[0], ip[1], ip[2], ip[3]);
		memset(&remote, 0, sizeof(remote));//初始化然后获取ip地址和端口
	...
	//域模式下
		...
		int ret = getaddrinfo((char*)buf, portaddr, NULL, &res);//服务到端口的转换，存入res
...
char* socks5_auth_get_user(int fd)//获取user名（sock5版本）
{
	unsigned char size;
...
char* socks5_auth_get_pass(int fd)//获取password（sock5版本）
{
	unsigned char size;
	readn(fd, (void*)&size, sizeof(size));
...
int socks5_auth_userpass(int fd)//验证username和password（sock5版本）
{
